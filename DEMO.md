...
- 左下**评估**：选**仿真场景**（单峰/多峰/偏斜峰）、方式（纯函数/模拟硬件+噪声/平均）；勾**安全限位**后可为**每个自变量**单独设范围；模拟硬件下可填**接口映射**（自变量↔执行器、目标↔功率计，为接真仪器占位）
- 点 **▶ 运行** → 指标 + **轨迹图**三视图：**收敛**（目标/变量 vs 评估次数，下方**算子色带**标出每段属于哪个算子）/ **1D**（单自变量→目标）/ **2D**（双自变量，颜色=目标值，等高线式）；**⭳ 下载轨迹** 导出全部 x/y 的 CSV
