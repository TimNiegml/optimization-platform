...
- **画布收件箱（重要）**：平台是服务器、你是客户端，平台**无法主动叫醒你**。用户在画布聊天框发的指令
  只会先存进会话，等你来取。所以**每次和用户交互一开始、以及长任务中途，都先 `poll_messages(session)`**
  把收件箱里的用户新指令读出来优先处理（读过即标记已读，只消费一次）。用户也可能直接在你的对话里下指令——
  两条通道都要照应。

## 1. 标准工作流（你的主循环）

0. **先清收件箱**：`poll_messages(session)` 看用户在画布上有没有发新指令；有就先处理它。
