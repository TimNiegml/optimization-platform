...
def _bayes_builder(vocs, step):
    ...
    return BayesianGenerator(vocs, step["variables"], step["objective"],
                             ...
                             search_ranges=step.get("search_ranges"),
...
_BUILTINS = [
    AlgorithmSpec(
        ...
        lambda v, s: GridScan(v, s["variables"], s["objective"],
                              ...
                              search_ranges=s.get("search_ranges")),
        {"n_per_axis": {"type": "int", "default": 7, "min": 3, "max": 21,
                        "label": "每轴取点数"},
         "span_frac": {"type": "float", "default": 0.0, "min": 0.0, "max": 1.0,
                       "label": "扫描窗口(相对起点)",
                       "hint": "0=全程绝对扫描；>0=以起点为中心、该比例的相对窗口；下方逐轴范围优先"},
         "search_ranges": {"type": "axis_ranges", "default": {}, "label": "每轴搜索范围(可空)",
                           "hint": "直接设置每个已选自变量的 [下限, 上限]；留空才使用相对窗口或设备全量程"}},
        group="find",
    ...
    AlgorithmSpec(
        ...
        lambda v, s: CoordinateDescent(v, s["variables"], s["objective"],
                                       ...
                                       tol_frac=s.get("tol_frac", 1e-3), steps=s.get("steps"),
                                       refine_step=s.get("refine_step", True),
                                       shrink_factor=s.get("shrink_factor", 0.5),
                                       shrink_patience=s.get("shrink_patience", 1)),
        {"step_frac": {"type": "float", "default": 0.25, "min": 0.01, "max": 1.0,
         ...
         "refine_step": {"type": "bool", "default": True, "label": "无提升时降低步距",
                         "hint": "关闭后，一个完整坐标扫描没有更优点就立即停在当前最优，不再缩小步距"},
         "shrink_patience": {"type": "int", "default": 1, "min": 1, "max": 20,
                             "label": "连续无提升轮数后降步距"},
         "shrink_factor": {"type": "float", "default": 0.5, "min": 0.05, "max": 0.95,
                           "label": "步距缩小倍数"},
         "tol_frac": {"type": "float", "default": 1e-3, "min": 1e-8, "max": 0.1,
                      "label": "最小步距比例", "hint": "所有轴步距小于各自量程×该值后停止"}},
        group="fine",
    ...
    AlgorithmSpec(
        ...
        lambda v, s: GradientAscent(v, s["variables"], s["objective"],
                                    ...
                                    tol_frac=s.get("tol_frac", 1e-3), max_line=s.get("max_line", 6),
                                    shrink_on_fail=s.get("shrink_on_fail", True),
                                    shrink_factor=s.get("shrink_factor", 0.5),
                                    grow_factor=s.get("grow_factor", 1.5)),
        {"step_frac": {"type": "float", "default": 0.15, "min": 0.01, "max": 0.5,
                       "label": "步长比例"},
         "probe_frac": {"type": "float", "default": 0.02, "min": 0.005, "max": 0.1,
                        "label": "探测步比例"},
         "shrink_on_fail": {"type": "bool", "default": True, "label": "失败时降低步长",
                            "hint": "关闭后，沿梯度试走没有提升就直接停在已有最优点"},
         "shrink_factor": {"type": "float", "default": 0.5, "min": 0.05, "max": 0.95,
                           "label": "失败步长缩小倍数"},
         "grow_factor": {"type": "float", "default": 1.5, "min": 1.0, "max": 3.0,
                         "label": "成功步长放大倍数"},
         "max_line": {"type": "int", "default": 6, "min": 1, "max": 50,
                      "label": "连续失败重试次数"},
         "tol_frac": {"type": "float", "default": 1e-3, "min": 1e-8, "max": 0.1,
                      "label": "最小步长比例"}},
        group="coarse",
    ...
    AlgorithmSpec(
        ...
        {"damping": {"type": "float", "default": 0.5, "min": 0.05, "max": 1.0,
                     "label": "阻尼比 d",
                     "hint": "缩放整次执行器修正 Δx；越大收敛越快但更易过冲，越小更稳但需要更多轮次"},
         "reg": {"type": "float", "default": 1e-6, "min": 0.0, "max": 1.0,
                 "label": "正则 λ",
                 "hint": "作用在灵敏度矩阵的逆上，重点压制弱/病态方向；越大越抗噪，但会增加求解偏差"},
         "tol": {"type": "float", "default": 1e-4, "min": 0.0, "max": 1.0,
                 "label": "残差容差", "hint": "残差范数低于此值时允许提前结束"},
         "max_solves": {"type": "int", "default": 40, "min": 1, "max": 1000,
                        "label": "最大修正次数",
                        "hint": "最多执行多少次 Δx 修正；达到残差容差可提前结束，实际测量点最多为次数+1"},
        ...
        group="solve",
        label="阻尼灵敏度求解",
        desc="多进多出定值：按规定的最大修正次数迭代。阻尼控制每轮走多远；正则抑制灵敏度矩阵的弱/病态方向。",
    ),
    AlgorithmSpec(
    ...
    AlgorithmSpec(
        ...
        {"sampler": {"type": "enum", "default": "tpe", "options": ["tpe", "gp", "random"],
         ...
         "span_frac": {"type": "float", "default": 0.0, "min": 0.0, "max": 1.0,
                       "label": "初始搜索范围(相对起点)",
                       "hint": "0=在全量程里搜；>0=只在以起点为中心、该比例的窗口内搜；下方逐轴范围优先"},
         "search_ranges": {"type": "axis_ranges", "default": {}, "label": "每轴搜索范围(可空)",
                           "hint": "直接设置每个已选自变量的 [下限, 上限]；留空才使用相对窗口或设备全量程"},
