...
- **数据变换**节点可将矩阵做转置、展平、ROI、行/列均值、统计量或归一化；标量统计输出可直接作为后续优化目标。NumPy 数组和 DataFrame 会在硬件入口自动转为 JSON 安全数据。
- 算法节点的目标编辑器提供“从数组元素快速创建目标”：向量填一个索引，矩阵填写行、列，平台自动插入元素提取节点并将其标量输出设为目标。
- 使用 `OPTPLAT_DEVICE=...` 启动后，画布会从 `/vocs` 自动加载设备声明的全部自变量、因变量、范围、当前位置、执行器/测量参数和数据类型；无需在界面重新录入。
- 顶部 **数据中心** 展示通道显示名、原始名、类型、shape、dtype、单位和来源；可“试读一次”确认真实格式，也可用两个向量的独立切片快速创建逐元素差值特征。设备模板可用 `display_name/unit/shape/dtype` 提供清晰元数据。
- 一维数据使用 `value_type="vector"`：可用向量观测仪画 1D 曲线、选择元素作为标量目标或参与向量变换。大量因变量场景下，数据中心、标量观测仪、组合目标和灵敏度选择器均提供搜索、滚动区域及全选/清空。
- 自动调优可直接勾选参与的自变量与标量因变量；对于“4 个目标、6 个候选轴选 4 个”的求解，可先采集完整灵敏度矩阵，再在自动调优中遍历组合，按满秩性和条件数排名并一键生成阻尼灵敏度求解节点。
- 阻尼灵敏度节点必须设置“最大修正次数”（默认 40，达到残差容差可提前停；测量点最多为次数+1）。阻尼 `d` 缩放每轮完整修正步长，控制速度与过冲；正则 `λ` 修改灵敏度矩阵的伪逆，压制弱/病态方向，控制噪声放大与求解偏差，两者不可互相替代。
- 网格扫描、线扫描和贝叶斯节点可逐轴勾选并填写绝对搜索下限/上限；未设置时才使用相对起点窗口，窗口为 0 时使用设备全量程。坐标下降和梯度上升也开放了降步距开关、缩小倍数、触发轮数/重试次数及停止步距；关闭降步距后，当前步距找不到更优点就直接停在已有最优点。
- 长通道列表的搜索支持“全选/清空筛选结果”：输入关键字后只批量处理当前匹配项，其他已选项保持不变；单项勾选不再重绘列表，因此滚动位置和搜索词不会跳回顶部或丢失。
...
编辑器可从下拉框点击插入已有因变量，并用 `+ − × ÷ ** abs/min/max` 按钮在光标处拼装表达式；应用前会检查非法字符、括号和未知通道，减少手输长名称导致的错误。

### 节点间观测仪

画布左栏提供三类可连线节点：**标量观测仪**（列出所选 y 的当前值）、**矩阵观测仪**（完整表格）和
**矩阵可视化观测仪**（选择每行或每列作为一个 feature 绘制曲线）。曲线可切换为保留原始形状的折线或 Spline 平滑样式，
并可为每个 feature 显示峰值位置虚线；把它插在两个算法节点之间，流程走到该节点时会在
